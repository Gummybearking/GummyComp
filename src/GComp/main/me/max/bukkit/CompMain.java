...
import GComp.main.me.max.bukkit.clearmine.ClearMine;
...
import GComp.main.me.max.bukkit.worldkick.WorldKick;
...
public class CompMain extends JavaPlugin {
	...
	public static WorldKick worldkick;
	public static ClearMine clearmine;
	...
	public void onEnable() {
		...
		if (!config.exists()) {
			...
			System.out
					.println("[GummyComp] No config.yml detected, config.yml created");
	...
	public void loadPlugins() {
		...
		// WorldKick
		worldkick = new WorldKick();
		worldkick.enable(this);
		// ClearMine
		clearmine = new ClearMine();
		clearmine.enable(this);
	...
	public boolean onCommand(CommandSender Sender, Command cmd, String label,
			...
			if (args.length == 1) {
				if (SilentHeal.isEnabled) {
					if (Bukkit.getPlayer(args[0]) != null) {
						silentheal.sheal((Player) Sender,
		...
		case "boss":
			...
			break;
		case "clearmine":
			if(ClearMine.isEnabled) {
				if(args.length == 1) {
					clearmine.clearMine((Player)Sender, Integer.valueOf(args[0]));
				} else {
					Sender.sendMessage("Usage: /ClearMine <radius>");
				}
			}
		case "gummycomp":
			if (args.length == 2 && Sender.isOp()) {
				...
				if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
					reloadConfig();
					Sender.sendMessage("GummyComp Config Reloaded");
				}
			}
		}
